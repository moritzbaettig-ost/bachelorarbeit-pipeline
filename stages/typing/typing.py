...
class RootNode(INode):
    ...
    def aggregate(self) -> None:
        ...
        """
        # Start aggregation in all child nodes
...
class DirNode(INode):
    ...
    def aggregate(self) -> None:
        ...
        Removes all entries older than 7 days from the timestamps_long_term list.
        """
        # Start aggregation in child nodes
    ...
    def update_reliability(self, parent_short_term_length: int, parent_medium_term_length: int, parent_long_term_length: int, calculated_reliability: float) -> None:
        ...
        if not self.core_node:
            ...
            # Check if node is up more than 1h and less than 24h
                ...
                self.reliability = (len(self.timestamps_short_term)+medium_term_length) / (parent_short_term_length+parent_medium_term_length)
            # Default Node is up more than 24h
            else:
                self.reliability = (len(self.timestamps_short_term)+medium_term_length+long_term_length) / (parent_short_term_length+parent_medium_term_length+parent_long_term_length)

        for c in self.children:
            c.update_reliability(len(self.timestamps_short_term), medium_term_length, long_term_length, calculated_reliability*self.reliability)
