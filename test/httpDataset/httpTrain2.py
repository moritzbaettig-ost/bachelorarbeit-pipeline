...
def send_data(p: pipeline.Pipeline):
    ...
    rel_path_train = "normalTrafficTest.txt"
    rel_path_anomaly = "anomalousTrafficTest.txt"
    ...
    try:
        ...
        for req in http_requests_anomaly:
            my_method, my_uri, my_header, dictRequest = _process_request(req)
            if my_method == 'GET' and 'http://127.0.0.1:80/tienda1/miembros/editar.jsp' in my_uri:
                if my_method == 'GET':
